...
为了简化操作，我们提供了 `start.bat` 脚本，整合了所有常用功能。
双击运行 `d:\Project\auto-wzry\start.bat`，即可通过菜单选择：
1.  **录制专家数据**：自动启动录制器。
2.  **BC 训练 (从头开始)**：第一次训练使用。
3.  **BC 训练 (断点续训)**：第二局及以后使用，大幅节省时间。
4.  **PPO 微调**：加载 BC 模型进行强化学习。
...
- 玩第二局、第三局... ：
  - 选 1 录制。
  - 录完选 3 ，然后 直接把上一轮训练好的模型文件拖进窗口 （脚本会自动识别路径），回车即可快速微调训练。
- 觉得模型差不多了，想让它自己悟 ：
  - 选 4 ，把最好的 BC 模型拖进去，开启 PPO 强化学习。
