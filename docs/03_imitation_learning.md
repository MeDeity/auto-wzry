...
| 阶段 | 场景描述 | 推荐操作 | 对应菜单选项 |
| :--- | :--- | :--- | :--- |
| **新手上路** | 刚开始项目，还没有任何数据。 | 1. 选 **[1]** 录制第一局游戏。<br>2. 录完选 **[2]** 进行初次 BC 训练。 | 1 -> 2 |
| **数据积累** | 已经玩过几局，觉得模型还不够聪明，想**多教几局**。 | 1. 选 **[1]** 录制新的一局（旧数据保留）。<br>2. 选 **[3]**，将上一轮最好的模型（如 `bc_model_epoch_50.pth`）拖入窗口，在已有基础上继续学习新数据，**节省时间**。 | 1 -> 3 |
| **强化进阶** | BC 模型已经能模仿基本操作，想让 AI **自我对战/探索**以变得更强。 | 1. 选 **[4]**，将最好的 BC 模型拖入窗口。<br>2. AI 将以此为起点，开启 PPO 强化学习。 | 4 |
| **长期挂机** | PPO 训练中途中断，或者想继续优化 PPO 模型。 | 1. 选 **[5]**，将最新的 PPO 模型（如 `ppo_wzry_epoch_100.pth`）拖入窗口继续训练。 | 5 |
...
⚠️ **重要警告：数据质量决定 AI 智商** ⚠️

1.  **操作窗口**：
    *   **必须**在脚本弹出的 `Expert Recorder` 窗口（有红点提示的那个）内进行鼠标点击/拖拽。
    *   **禁止**直接操作 Scrcpy 原生窗口或手机屏幕，否则脚本无法捕获你的动作，导致 AI 学会“挂机”。

2.  **录制姿势**：
    *   先按 `R` 开始录制 -> **松开鼠标** -> 再按下鼠标开始操作。
    *   **切勿**先按住鼠标不放（如一直跑路），然后再按 `R`。这会导致“按下”事件丢失，AI 会认为你一直在发呆。

3.  **短片段原则**：
    *   **只录高光时刻**：比如“完美的一套连招”、“正确的清线节奏”。
    *   **跳过垃圾时间**：不要录制挂机、回城买装备、死亡等待等无效时间。模仿学习是“照猫画虎”，如果你录了发呆，AI 也会学会发呆。
    *   **多片段支持**：可以在一局游戏中多次按 `R` 录制不同的小片段（出门、打野、团战），脚本会自动保存为多个文件。

4.  **针对性补课**：
    *   如果发现 AI 不会推塔，就专门进游戏录制几次推塔的片段。
    *   如果发现 AI 团战秒躺，就专门录制团战拉扯的片段。
